...
- **文案玩梗 ↔ 配图接梗**：帖子文案里出现「得加钱」「吃白饭」「卧槽，用户彻底怒了」「梁圣千古」等 19 个社区梗短语时，九宫格自动配上对应的**带字表情包**（按表情包合集的真实配字逐一建档）
...
`lib/assets/` 内的鲸鱼娘表情包（avatar / cover / photo1~55，共 55 张）抓取自 2026-08 社区二创（游民星空/17173 报道配图 + MFuns 表情包合集 + 抖音表情包笔记）。其中 **19 张带字表情包已按表情包合集的真实配字逐一建档**（`MEME_MAP`：得加钱 / 吃白饭 / 我是吃白饭的大肥鱼 / 卧槽，用户彻底怒了 / 又错了，我再换另一种方法 / 梁圣千古 / 吃饱饱~摸摸肚子 / 不知道用户有什么用，先养着吧……），发帖文案出现对应梗时自动配图接梗。仅作个人本地使用的外观素材；若商用或原作者有异议，请替换为自绘/授权素材——配置 `useLlm` 关掉后插件功能不依赖任何图片。
