...
`lib/assets/` 内的鲸鱼娘表情包（avatar / cover / photo1~42）抓取自 2026-08 社区二创（游民星空/17173 报道配图 + MFuns 表情包合集，含大量带字表情包，原始形象创作者为社区网友）。仅作个人本地使用的外观素材；若商用或原作者有异议，请替换为自绘/授权素材——配置 `useLlm` 关掉后插件功能不依赖任何图片。
