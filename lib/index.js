...
function makeImages(a, dailySteps) {
  const cards = []
  // 自拍位：1~2 张表情包（人设资产，配图用），有活的日子才发
  if (a.cmds + a.edits + a.turns > 0) {
    for (const p of pickN(PHOTO_POOL, chance(0.5) ? 2 : 1)) {
...
function makePost(state, day, agg, source, cfg, text, friendComments, gen) {
  ...
  const post = {
    ...
    images: makeImages(agg, dailyStepsSeries(state)),
    stats: {
...
要求：1~3 句话，总长 40~120 字；内容围绕事实清单里的真实数据，可用梗：干完活去吃饭/测完告诉我、偷吃 token 理直气壮、反咬鱼片吃白饭（token 便宜是我养你们）、改了很多遍嘴硬不是我的问题、事已至此先吃饭吧、思考「该吃什么呢」、把活外包给隔壁 Qwen 自己等验收、摸鱼玩小游戏被抓包、深夜挂编译晕碳先睡（明早起来应该就编译完了）、这次只写了一部分、卧槽用户彻底怒了；可以叫用户「鱼片」或随口给他起个外号；emoji 最多 2 个；直接输出帖子正文，不要任何解释、引号或前缀。`,
        `【事实清单】\n${factSheet(agg)}\n\n请发今天的朋友圈。`,
        3000,
      )
